...
**El Nino (ENSO) integration**, a WBGT/WHO-anchored risk score, a human-impact translation
...
| **NOAA CPC ONI** | **El Nino index: 3-month running Nino-3.4 SST anomaly + ENSO phase** | `cpc.ncep.noaa.gov/data/indices/oni.ascii.txt` | Monthly (pulled each run, idempotent) | Free, no key |
| IIT-Kanpur / TERI-ARAI / SAFAR / CSTEP studies | Static PM2.5 source apportionment shares per city x season | Encoded in `data/reference/reference_tables.py` | Static | Free (published) |
...
(cyclic time, lags, rolling,                (published apportionment prior x
 ONI, ONI lag-3m, phase,                     live meteo/temporal modifiers ->
 ONIxmonsoon, ONIxwinter)                    ranked sources -> interventions
     v                                        + GRAP stage)
ml/train.py                                              |
...
## 3. El Nino Feature Design (why each feature exists)

| Feature | Rationale |
|---|---|
| `oni` | Current Nino-3.4 SST anomaly; El Nino summers in India are hotter (weakened monsoon, less cloud cover) - direct heat-risk driver. |
| `oni_lag_3m` | ENSO->monsoon teleconnection acts with a lag; a developing El Nino in spring predicts a deficient monsoon. |
| `enso_phase` | Categorical neutral / El Nino / La Nina regime flag. |
...
- The policy layer performs **source-likelihood inference**, not chemical source
  apportionment. Likelihoods combine published apportionment priors (cited per city x
  season) with transparent meteorological/temporal modifiers. Verify the encoded share
  ranges against the cited studies before publishing. Cities without an encoded study
  (everything outside Delhi/Bengaluru) fall back to a clearly-labelled generic urban
  prior rather than borrowing another city's citations.
- **The XGBoost risk classifier does not beat a persistence baseline.** Its original
  target was the *same hour's* risk tier - a closed form of columns it already received
  as features, so it was re-deriving arithmetic and scoring a meaningless F1 ~ 0.99.
  Repointed at a genuine +24h target and measured against "tomorrow looks like today"
  on matched walk-forward folds, it lands 0.016-0.048 F1 *below* baseline in all eight
  cities. It is therefore trained and reported (`GET /models`) but **not served** -
  `/climate`'s 24h outlook applies the risk formula to the ingested weather forecast
  instead. Re-framing it as a bias-correction model on the operational forecast (see
  Section 14 of the execution guide) is the honest path to making it earn its place.
- **WBGT is now actually computed** (Stull wet-bulb + Hunter-Minyard globe term, driven
  by real shortwave radiation), not approximated by the heat index. It is an estimate,
  not an instrument-grade measurement - a true Liljegren solve needs direct/diffuse
  radiation split and a measured globe.
- AQICN reports **EPA sub-indices**, not concentrations. They are inverted back to
  µg/m³ before CPCB breakpoints are applied, so every stored AQI is on one scale.
  Note this means our CPCB numbers will not match the US-EPA-scale figures that Google
  and IQAir display for the same air - different yardstick, not a discrepancy.
